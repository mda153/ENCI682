...
if SectionCurvatureDuctility > 4:
    ...
    if -steelstrain[-1] >= esfl[-1]:
        ...
        plt.plot(failCuDuMK, failss, 'm', marker='.', MarkerEdgeColor='k', mfc='g', MarkerSize=12,
                 label=None)
...
if AxialRatio >= 0.30:
    ...
    C2 = 3.129;
    C3 = 0.651;
    C4 = 0.227;  # model constants
else:
    C0 = 0.0010;
    C1 = 7.30;
    C2 = 1.30;
    C3 = 1.30;
    C4 = 3.00;  # model constants
...
if max(plrot) > rotb:
    ...
    plt.plot(failCuDuBE, failplrot, linestyle='', marker='.', MarkerEdgeColor='k', mfc='g', MarkerSize=12,
             label='Buckling')
...
kscr = ((0.39 * TransvSteelRatio) * 0.25 * Es * ((0.8 * D / 1000) ** 2) / (
...
Vsd = (0.5 * np.pi * (0.25 * np.pi * (Dh ** 2)) * fyh / np.tan((35 / 180) * np.pi) * (
            D - clb + 0.5 * Dh - cMn) / s) / 1000
beta = min(0.5 + 20 * LongSteelRatio, 1)
Dductf = displ / dyf
...
Bi = 1 / (((mombilin[1]) / (curvbilin[1])) / (
            (mombilin[2] - mombilin[1]) / (curvbilin[2] - curvbilin[1])))  # Bilinear factor
...
if criteria != 1 and (bucritMK == 1 and bucritBE == 0):
    ...
    ax.plot(faildispl, failforce, c='magenta', linestyle='', marker='.', MarkerEdgeColor='black', mfc='red',
            markersize=15, label='shear failure')
    ax.plot(buckldispl, bucklforce, c='red', linestyle='', marker='*', MarkerEdgeColor='black', mfc='red',
            markersize=15, label='buckling (M & K)')
    ...
    ax.plot(buckldispl, bucklforce, c='red', linestyle='', marker='*', MarkerEdgeColor='black', mfc='red',
            markersize=15, label='buckling (M & K)')
    ax.plot(buckldisplBE, bucklforceBE, c='red', linestyle='', marker='s', MarkerEdgeColor='black',
            mfc='red', markersize=15, label='buckling (B & E)')
    ...
    ax.plot(faildispl, failforce, c='magenta', linestyle='', marker='.', mec='black', mfc='red',
            markersize=15, label='shear failure')

elif criteria == 1 and (bucritMK == 1 and bucritBE == 0):
    buckldispl = interp1d(CuDu, displ)(failCuDuMK)
    bucklforce = interp1d(CuDu, Force)(failCuDuMK)
    ax.plot(buckldispl, bucklforce, c='red', linestyle='', marker='*', mec='black', mfc='red',
            markersize=15, label='buckling (M & K)')
    ...
    ax.plot(buckldispl, bucklforce, c='red', linestyle='', marker='*', mec='black', mfc='red',
            markersize=15, label='buckling (M & K)')
    ax.plot(buckldisplBE, bucklforceBE, c='red', linestyle='', marker='s', mec='black',
            mfc='red', markersize=15, label='buckling (B & E)')
...
ax.fill_between(displ, 0, Force, edgecolor='black', facecolor=(0.4, 0.8, 0.6), interpolate=True, alpha=0.4,
                label='ultimate zone')
ax.fill_between(displ[pointsdam], 0, Force[pointsdam], edgecolor='black', facecolor=(0.4, 0.6, 0.6), interpolate=True,
                alpha=0.4, label='damage control zone')
ax.fill_between(displ[pointsser], 0, Force[pointsser], edgecolor='black', facecolor=(0.4, 0.4, 0.6), interpolate=True,
                alpha=0.4, label='serviceability zone')
ax.grid(True)
...
PCid = interp1d(ec, fc)(csid) * (Acore - AsLong) + interp1d(ecun, fcun)(csid) * (Agross - Acore) + AsLong * interp1d(es,
                                                                                                                     fs)(
    csid)
...
for i in range(output.shape[0]):
    fid.write(
        '%1.5f\t%1.5f\t%4.2f\t%1.5f\t%8.2f\t%1.5f\t%8.2f\t%1.5f\t%1.5f\t%1.5f\t%8.2f\t%8.2f\n' % tuple(output[i, :]))
fid.write('\n')
...
for i in range(nPP):
    ...
    for k in range(npts):
        ...
        while abs(F) > tol:
            ...
            fcunconf = interp1d(ecun, fcun, fill_value="extrapolate")(
                eec)  # vector with stresses in the unconfined concr.
        ...
        DF.append(x[niter])
        vniter.append(niter)
        temp = (sum(FUNCON * conclay[:, 0]) + sum(FCONF * conclay[:, 0]) + sum(FST * distld) - PP[i] * (D / 2)) / (
                    10 ** 6)
        mom.append(temp)
...
plt.plot(Mni, PPn / 1000, c='red', linestyle='-', marker='o', mec='black', mfc='red',
         markersize=6, label='Interaction Diagram')
plt.plot(MnL, PPL / 1000, c='blue', linestyle='--', marker='s', mec='black', mfc='blue',
         markersize=6, label='Approximation for NLTHA')
...
plt.figure(figsize=(8, 6))
plt.plot(PPn[1:-1] / 1000, eci, c='blue', linestyle='--', marker='s', mec='black', mfc='blue',
         markersize=6)
...
plt.figure(figsize=(8, 6))
plt.plot(PPn[1:-1] / 1000, esi, c='blue', linestyle='--', marker='s', mec='black', mfc='blue',
         markersize=6)
plt.xlabel('Axial Force [kN]', fontsize=16)
