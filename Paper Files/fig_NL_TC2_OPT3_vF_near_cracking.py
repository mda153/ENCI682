...
def get_rc_fibre_section(osi, conc_conf, conc_unconf, rebar, nf_core_y, nf_core_z, nf_cover_y, nf_cover_z):
    ...
    """
    # import numpy as np
    fc = 30e6 #[Pa] concrete compressive strength
    fy = 500.0e6  #[Pa] steel yield strength
    Es = 200.0e9  # [Pa] Steel modulus of elasticity 
    depth = 0.6  #[m] section depth
    width = 0.3 #[m] section width
    
    


    col_sect = sm.sections.RCDetailedSection(depth=depth, width=width)
    col_sect.layer_depths = [0.05, 0.55]  # 44mm cover (m units)
    col_sect.bar_diams = [[0.016, 0.016], [0.016, 0.016]]  # 12mm bars - two 12mm bars directly beside each other at top of section
    col_sect.bar_centres = [[0.05, 0.25], [0.05, 0.25]]
...
def create(): #creates plot
    ...
    :return:
    """
    bf, ax = plt.subplots(nrows=2, squeeze=True, sharex='col', figsize=(6, 4))
    #grabs relevant values from run function
    stypes = ["Non-Linear", "Non-Linear", "Non-Linear"]
    cols = ['k', 'k', 'k']
    lss = ['-', '--', ':']
    ...
    for ss, stype in enumerate(stypes):
        ...
        ax[0].plot(nx, (diff_disp - diff_disp[ind]) * 1e3, label=f'{lab}', c=cols[ss], ls=lss[ss])
        ...
        ax[1].plot(nx, moment / 1e3, label=f'{lab}', c=cols[ss], ls=lss[ss])
    ...
    # ax[1].axhline(m_crack / 1e3, ls='-.', label="Cracking Moment", c="g")
    ax[1].axhline(-m_crack / 1e3, ls='-.', label="Cracking Moment", c="g")
    ax[1].axhline(m_y / 1e3, ls="-.", label='Yield Moment', c="orange")
    ax[1].axhline(-m_y / 1e3, ls="-.", label='Yield Moment', c="orange")
    # ax[1].axhline(-m_ult / 1e3, ls="-.", label="Ultimate Moment", c="k")
    
    # ax[0].set_xlabel('Foundation Length (m)')
    ax[0].set_ylabel('Displacement [mm]')
    ax[1].set_ylabel("Moment [kNm]")
    ax[-1].set_xlabel("Foundation Beam Length [m]")
    ...
    bf.savefig('moment_drop.png', dpi=400)
    plt.show()
