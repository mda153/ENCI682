...
def run(lf, dx, xd, yd, ksoil, udl, num_incr, stype="Non-Linear"):
    ...
    x_incrs = np.arange(0, L+dx, dx, dtype=None)
    print(x_incrs)
...
def create(): #creates plot
    ...
    for ss, stype in enumerate(stypes):
        nx, node_disps, forces, xd0n, xd1n, yd0, yd1,mom, shear, ndisps, spring_forces, M_fix, V_fix, disp_fix, M_pin, V_pin, disp_pin, x_incrs = run(lf=lf, dx=dx, xd=xd, yd=(support_disp,support_disp), ksoil=3.3333*1e7, udl=udl, num_incr=500, stype=stype)
