...
def get_moment_curvature(axial_load=0, max_curve=0.25, num_incr=100):
    ...
    print("EC:",Ec)
    d_bar = 0.016 # longtitudinal bar diameter [m]
    total_num_bars = 4
    Ast = np.pi*(d_bar/2)**2*total_num_bars #Total area of longtitudinal steel (m^2)
    Dh = 0.012 #diameter of transverse reinforcement (m)
    clb = 0.05 #cover to longtitudinal bars (m)
    s = 0.45 #spacing of transverse steel (m)
    fy = 500e6 #Pa
...
if __name__ == '__main__':
    ...
    mom, curve ,mom_crack, mom_yield = get_moment_curvature(num_incr=4000, max_curve=0.35)
    ...
    np.savetxt('m_curv_opensees_vTEST_TC2_Opt3.csv', (mom_list, curve_list), fmt='%.4e', delimiter=',')# X is an array
