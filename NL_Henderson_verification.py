...
def get_rc_fibre_section(osi, conc_conf, conc_unconf, rebar, nf_core_y, nf_core_z, nf_cover_y, nf_cover_z):
    ...
    col_sect.layer_depths = [0.044, 0.556]  # 44mm cover
    col_sect.bar_diams = [[0.024], [0.012, 0.012]]  # 12mm bars - two 12mm bars directly beside each other at top of section - model this as 1 x 24mm bar
    col_sect.bar_centres = [[0.044], [0.556, 0.556]]
...
def run(lf, dx, xd, yd, ksoil, udl, axial_load, max_curve, num_incr, stype="rc"):
    ...
    Ec = 5000 * np.sqrt(fpc)  # Concrete modulus of elasticity (Pa) - eqn in Mander script for auto calc
    ...
    print("hi x trib", x_trib, nx)
    
    
    # ks = ksoil * s_width * spring_spacing
    ...
    ks = [2500000,2500000,2500000,2500000,2500000,2500000,2500000,2500000,2500000,2500000,2500000,2500000,2500000,2500000,2500000,2500000,2500000,2500000,2500000,2500000,2500000]
    
    print("hi k_s", ks)
    
    py = 1.0e3 * x_trib * s_width  # N Capacity (TODO: define this better, see Henderson thesis for details) spring force resistance
    ...
    # for j in range(nnodes - 1):
        # sh[0].append(o3.get_ele_response(osi, fd_eles[j], 'force')[4])
    print('hi', ndisps)
    for i in range(max_steps + 20):
        ...
        # for j in range(nnodes-1):
            # sh[-1].append(o3.get_ele_response(osi, fd_eles[j], 'force')[4])
    ...
    forces = efr.collect()
    return nx, node_disps, forces, xd0n,xd1n, yd0, yd1, mom, sh
...
def create(): #creates plot
    ...
    for ss, stype in enumerate(stypes):
        nx_elastic, ndisps_elastic, forces_el, xd0n_elastic, xd1n_elastic, yd0_elastic, yd1_elastic,mom_el,sh_el = run(lf=10, dx=0.5, xd=(3,7), yd=(-0.1,-0.1), ksoil=25000e3, udl=64.6, axial_load=0, max_curve=0.003, num_incr=500, stype=stype)
        ...
        # ax[2].plot(el_x, sh_el[-1], label=f'{stype}', c=cols[ss])
