...
def get_moment_curvature(axial_load=0, max_curve=0.01, num_incr=5000):
    osi = o3.OpenSeesInstance(ndm=2, ndf=3, state=3)
    
    fpc = 17.2e0 #Conc compressive strength (MPa)
    ...
    Ast = 452.4 #Total area of longtitudinal steel (m)
    # Dh = 6 #diameter of transverse reinforcement (m)
    clb = 44 #cover to longtitudinal bars (m)
    ...
    # section = 1
    # D = 1
    d = 600 #section depth (m)
    b = 130 #section width (m)
...
if __name__ == '__main__':
    ...
    plt.plot(curve, mom/1000/1000, label="Moment Curvature Analysis")
    ...
    plt.axhline(m_cr, c="green", label="CUMBIA Crack Moment")
    plt.axhline(m_y, c="orange", label="CUMBIA Yield Moment")
    plt.axhline(m_ult, c="red", label="CUMBIA Ultimate Moment")
