...
def run(lf=10, dx=0.5, s_depth=0.6, xd=(4,6), yd=(-0.2,-0.2), ksoil=2.5e6, udl=42.9):
    """
    ...
    s_width = 0.2  # [m] section width
    imposed_displacement = 0.2 #[m]
    ...
    for j in range(nnodes-1):
        shear[0].append(o3.get_ele_response(osi, fd_eles[j], 'force')[1])
    ...
    for i in range(100):
        ...
        for j in range(nnodes-1):
            # print('force: ', o3.get_ele_response(osi, fd_eles[j], 'force'))
            shear[-1].append(o3.get_ele_response(osi, fd_eles[j], 'force')[1])
            
       
            
        print('mom', mom, ndisps)
