...
def get_moment_curvature(axial_load=100., max_curve=0.001, num_incr=500):
    ...
    fc = 4.0
    e_mod = 57000.0 * np.sqrt(fc * 1000.0) / 1e3 #Is not used in analysis at the moment - Should be??...

    conc_conf = o3.uniaxial_material.Concrete01(osi, fpc=-4.0, epsc0=-0.005, fpcu=-3.5, eps_u=-0.02) #Confined concrete paramters
    conc_unconf = o3.uniaxial_material.Concrete01(osi, fpc=-fc, epsc0=-0.002, fpcu=0.0, eps_u=-0.006) #Unconfined concrete parameters
    rebar = o3.uniaxial_material.Steel01(osi, fy=60.0, e0=30000.0, b=0.02) #Steel reinforcing properties
    
    #Defining concrete beam parameters

    h = 18.0 #Section height
    b = 18.0 #Section width
    cover = 2.5 #Cover depth
    gj = 1.0E10 # Is this G * J??
    nf_core_y = 8
    nf_core_z = 8
    nf_cover_y = 10
    nf_cover_z = 10
    n_bars = 3 #Number of bars
    bar_area = 0.79 #Area of steel (inches^2)
